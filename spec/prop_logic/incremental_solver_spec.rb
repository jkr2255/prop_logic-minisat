...
describe PropLogic::Minisat::IncrementalSolver do
  ...
  expected_methods.each do |sym|
    it "has ##{sym} method" do
      expect(PropLogic::Minisat::IncrementalSolver.public_instance_methods).to include sym
    end
  end

  describe '#add' do
    it 'can add one term properly' do
      s = PropLogic::Minisat::IncrementalSolver.new a
      s.add(b)
